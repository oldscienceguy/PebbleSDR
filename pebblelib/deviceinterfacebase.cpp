									 ...
									 quint16 _framesPerBuffer)
{
	...
	m_audioInputBuffer = memalign(m_framesPerBuffer);
...
void DeviceInterfaceBase::normalizeIQ(CPX *cpx, float I, float Q)
{
	...
	cpx->real(I * m_userIQGain * m_normalizeIQGain);
	cpx->imag(Q * m_userIQGain * m_normalizeIQGain);
...
void DeviceInterfaceBase::normalizeIQ(CPX *cpx, qint16 I, qint16 Q)
{
	double tmp;
	//Normalize and apply gain
	cpx->real((I / 32768.0) * m_userIQGain * m_normalizeIQGain);
	cpx->imag((Q / 32768.0) * m_userIQGain * m_normalizeIQGain);
...
void DeviceInterfaceBase::normalizeIQ(CPX *cpx, quint8 I, quint8 Q)
{
	double tmp;
	//Normalize and apply gain
	cpx->real(((I - 128) / 128.0) * m_userIQGain * m_normalizeIQGain);
	cpx->imag(((Q - 128) / 128.0) * m_userIQGain * m_normalizeIQGain);
...
void DeviceInterfaceBase::normalizeIQ(CPX *cpx, qint8 I, qint8 Q)
{
	double tmp;
	//Normalize and apply gain
	cpx->real((I / 128.0) * m_userIQGain * m_normalizeIQGain);
	cpx->imag((Q / 128.0) * m_userIQGain * m_normalizeIQGain);
	...
	switch(tmpOrder) {
		case DeviceInterface::IQO_IQ:
			for (quint32 i=0; i < _numSamples; i++) {
				_out[i].real(_in[i].real() * scale);
				_out[i].imag(_in[i].imag() * scale);
			}
			break;
		case DeviceInterface::IQO_QI:
			for (quint32 i=0; i < _numSamples; i++) {
				_out[i].real(_in[i].imag() * scale);
				_out[i].imag(_in[i].real() * scale);
			}
			break;
		case DeviceInterface::IQO_IONLY:
			for (quint32 i=0; i < _numSamples; i++) {
				_out[i].real(_in[i].real() * scale);
				_out[i].imag(_out[i].real());
			}
			break;
		case DeviceInterface::IQO_QONLY:
			for (quint32 i=0; i < _numSamples; i++) {
				_out[i].imag(_in[i].imag() * scale);
				_out[i].real(_out[i].imag());
			}
			break;
	}
}
...
void DeviceInterfaceBase::normalizeIQ(CPX* _out, CPXU8* _in, quint32 _numSamples, bool _reverse)
{
	...
	switch(tmpOrder) {
		case DeviceInterface::IQO_IQ:
			for (quint32 i=0; i < _numSamples; i++) {
				_out[i].real((_in[i].real() - 128.0) * scale);
				_out[i].imag((_in[i].imag() - 128.0) * scale);
			}
			break;
		case DeviceInterface::IQO_QI:
			for (quint32 i=0; i < _numSamples; i++) {
				_out[i].real((_in[i].imag() - 128.0) * scale);
				_out[i].imag((_in[i].real() - 128.0) * scale);
			}
			break;
		case DeviceInterface::IQO_IONLY:
			for (quint32 i=0; i < _numSamples; i++) {
	...
	switch(tmpOrder) {
		case DeviceInterface::IQO_IQ:
			for (quint32 i=0; i < _numSamples; i++) {
				...
				_out[i].imag(_inQ[i] * scale);
			}
			break;
		case DeviceInterface::IQO_QI:
			for (quint32 i=0; i < _numSamples; i++) {
				_out[i].real(_inQ[i] * scale);
				_out[i].imag(_inI[i] * scale);
			}
			break;
		case DeviceInterface::IQO_IONLY:
			for (quint32 i=0; i < _numSamples; i++) {
