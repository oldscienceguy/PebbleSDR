...
/*
Support for RF Space SDR-IQ receiver
Thank you to Frank Kienast (KG9NZ) for helping decipher SDR-IQ protocol in his C# example
And Moe Wheatley AE4JY  <http://www.moetronix.com> for ActiveX app example
...
public:
	...
	SDR_IQ(Receiver *_receiver,SDRDEVICE dev,Settings *settings);
	~SDR_IQ(void);
	//SDR class overrides
	bool Connect();
	bool Disconnect();
	...
	unsigned StatusCode();
	QString StatusString(BYTE code);

	void ReadSettings();
	void WriteSettings();

	//SDR-IQ ReceiverControlItems
	//5.2.1 ReceiverState (come back to this)
	//double SetFrequency(double f);
	double GetFrequency();
	//5.2.3 SetSampleRate (skip this)
